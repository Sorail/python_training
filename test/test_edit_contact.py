...
def test_edit_some_contact(app, db, check_ui):
    ...
    with pytest.allure.step('When I edit a contact: %s to set new pram from contact: %s' % (contact, contact_new)):
        app.contact.edit_by_id(contact_new, contact.id)
    with pytest.allure.step('Then the new contact pram list is equal to the old list with the change old contact'):
